...
pub fn safe_versioned_state_for_testing(
    block_state: CachedState<HashSet<usize>, DictStateReader, VisitedPcsSet>,
...
pub fn create_fee_transfer_call_info<T, S: StateReader, V: VisitedPcsTrait<T>>(
    state: &mut CachedState<T, S, V>,
    account_tx: &AccountTransaction,
    concurrency_mode: bool,
) -> CallInfo {
    let block_context = BlockContext::create_for_account_testing();
    let mut transactional_state: TransactionalState<'_, T, _, V> =
